#!/usr/bin/env python3
"""
Gemini CLI Adapter — Universal Agent Memory API client.

Wrapper simplu care permite Gemini CLI (sau orice agent Google)
...
Exemplu:
    from adapters.gemini_cli_adapter import GeminiMemoryAdapter
...
class GeminiMemoryAdapter:
    """Client adaptor pentru Universal Agent Memory API (Google/Gemini)."""

    def __init__(
        self,
        base_url: str = "http://localhost:19876",
        project_path: str = "",
        cli_name: str = "gemini-cli",
        agent_name: str = "gemini-agent",
        provider: str = "google",
    ...
    # Expose _get/_post for tests that use them directly (e.g. t10_dashboard)
